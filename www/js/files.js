...
function init_files_panel(dorefresh) {
  if (target_firmware == "smoothieware") {
    ...
    document.getElementById("files_refresh_primary_sd_btn").innerHTML =
      primary_sd.substring(0, primary_sd.length - 1);
    document.getElementById("files_refresh_secondary_sd_btn").innerHTML =
      secondary_sd.substring(0, secondary_sd.length - 1);
    if (primary_sd.toLowerCase() != "none")
      document.getElementById("files_refresh_primary_sd_btn").style.display =
        "inline";
    if (secondary_sd.toLowerCase() != "none")
      document.getElementById("files_refresh_secondary_sd_btn").style.display =
        "inline";
...
function files_build_file_line(index) {
  ...
  if ((files_filter_sd_list && entry.isprintable) || !files_filter_sd_list) {
    ...
    if (files_showdeletebutton(index)) {
      content +=
        "<button class='btn btn-xs btn-danger' onclick='files_delete(" +
        index +
        ")'  style='padding-top: 4px;'>" +
        get_icon_svg("trash", "1em", "1em") +
        "</button>";
